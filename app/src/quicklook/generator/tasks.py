...
from quicklook.generator.tilewriter import TileWriterBase, TileWriter
from quicklook.storage import s3_get_visit_ccds
from quicklook.tileinfo import TileInfo
from quicklook.types import CcdId, PreProcessedCcd, Progress, Tile, Visit
...
def run_generator(task: GeneratorTask, on_update: Callable[[GeneratorProgress], None] | None = None):
        ...
        with timeit('generator'):
            with mp.Pool(config.tile_ccd_processing_parallel) as pool:
                with GeneratorProgressReporter(task, on_update=on_update) as progress:
                    ...
                    for _ in pool.imap_unordered(process_ccd, args()):
                        pass
...
def process_ccd(args: ProcessCcdArgs):
    ...
    def download(visit: Visit, ccd_name: str):
        with sem:
            with timeit(f'download-{visit.name}/{ccd_name}'):
                filecontents = s3_get_visit_ccds(visit, ccd_name)
