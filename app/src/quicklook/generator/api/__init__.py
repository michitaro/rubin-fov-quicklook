...
visit_ccd_names = LRUDict[Visit, set[str]](config.quicklook_meta_cache_size)
...
def get_tile(
    ...
    assert len(ccd_names) > 0
    pool: numpy.ndarray | None = None
    for ccd_name in ccd_names:
        ccd_id = CcdId(visit, ccd_name)
        if Path(f'{config.tile_tmpdir}/{ccd_id.name}/{z}/{y}/{x}.npy').exists():
            arr = numpy.load(f'{config.tile_tmpdir}/{ccd_id.name}/{z}/{y}/{x}.npy')
            if pool is None:
                pool = arr
