...
def test_preprocess_ccd_raw():
    ...
    with timeit('load-fits'):
        file_contents = s3_download_object(config.s3_test_data, f'raw/broccoli/R00_SG0.fits')
...
def test_preprocess_ccd_calexp():
    ...
    with timeit('load-fits'):
        file_contents = s3_download_object(config.s3_test_data, f'calexp/192350/R01_S00.fits')
